...
interface DeleteHydrationModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function DeleteHydrationModal({
  isOpen,
  onClose,
  entry,
  onSuccess,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to delete hydration entry");
        setIsLoading(false);
        return;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md bg-background-surface border border-border-default rounded-3xl p-6 shadow-2xl space-y-5 text-left">
        ...
        <div className="space-y-2 text-sm text-foreground-secondary">
          <p>
            Are you sure you want to remove this <strong className="text-foreground-primary font-bold">{entry.amountMl} ml {bevName}</strong> entry?
          </p>
          <p className="text-xs text-foreground-muted">
            Today&apos;s total hydration volume will automatically decrease by {entry.amountMl} ml.
          </p>
        </div>
...
export default DeleteHydrationModal;
