...
import { X, Activity, Gauge, Clock, Flame, Footprints, Mountain, Loader2, Save, AlertCircle, Bike } from "lucide-react";
...
export function EditRunModal({
  ...
  useEffect(() => {
    if (activity) {
      ...
      setRunningType(activity.runningType || "EASY");
      setDistanceKm(String(activity.distanceKm));
      const mins = Math.floor(activity.movingDurationSeconds / 60);
      const secs = activity.movingDurationSeconds % 60;
      ...
      setElevationGainMeters(String(activity.elevationGainMeters));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to update activity record.");
        return;
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 animate-fade-in">
      <div className="relative w-full max-w-lg bg-background-surface border border-border-default rounded-3xl p-6 sm:p-7 shadow-2xl space-y-5 text-left max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-foreground-muted">
                Update workout telemetry and metrics
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Dynamic Pace or Speed Calculation */}
          {activityType === "RUN" && (
            <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Gauge className="h-4 w-4 text-amber-400" />
                <span className="text-xs font-bold text-foreground-secondary">Recalculated Pace:</span>
              </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Felt strong throughout"
